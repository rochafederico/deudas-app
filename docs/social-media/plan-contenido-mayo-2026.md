# Plan de Contenido en Redes Sociales — Nivva

> **Período:** Mayo 2026  
> **Plataforma objetivo:** Instagram (adaptable a otras redes)  
> **Cadencia:** 3 publicaciones por semana (lunes, miércoles, viernes)  
> **Total de publicaciones:** 12  
> **Archivo de planificación:** Gestionado en Google Drive

---

## 📌 Pilares de contenido

| # | Pilar | Descripción |
|---|---|---|
| 1 | **Presentación de marca** | Quién es Nivva, su propósito y propuesta de valor |
| 2 | **Funcionalidad destacada** | Cómo se usa una feature específica de la app |
| 3 | **Educación financiera simple** | Conceptos básicos sin tecnicismos, útiles para los 3 perfiles |
| 4 | **Identificación con el usuario** | Posts relacionables que reflejan los pain points reales |
| 5 | **Privacidad y confianza** | Datos locales, sin servidores externos |
| 6 | **Recap y CTA** | Resumen de valor y llamado a la acción para nuevos usuarios |

---

## 🗓️ Tabla por Semana

### Semana 1 — 4 al 8 de mayo de 2026
> **Foco:** Presentación de Nivva e introducción al registro de egresos

| # | Fecha | Día | Pilar | Formato | Hook | CTA | Persona objetivo |
|---|---|---|---|---|---|---|---|
| 1 | 04/05 | Lunes | Presentación de marca | Reel (8–12 seg) | ¿Cuánto vas a pagar este mes? Si no lo sabés de memoria, Nivva te lo muestra. | Probala gratis → link en bio | Ana / Claudia |
| 2 | 06/05 | Miércoles | Funcionalidad destacada | Carrusel (3 slides) | De 0 a 'sé exactamente lo que debo' en 3 pasos | Guardá este post para acordarte 💾 | Facundo |
| 3 | 08/05 | Viernes | Identificación con el usuario | Post estático | Confiar en la memoria para recordar los vencimientos es como... 🫠 | ¿Te pasó? Contanos en los comentarios 👇 | Facundo / Claudia |

**Captions desarrollados:**

**Post 1 (04/05):**
> Nivva es la app gratuita para registrar tus egresos, ingresos e inversiones — todo desde tu celular, sin que tus datos salgan de tu dispositivo.
>
> Sin suscripciones. Sin cuentas. Solo vos y tus registros.
>
> Probala en el link de la bio 👇
>
> `#Nivva #finanzaspersonales #controlfinanciero #egresos #deudas #Argentina #appgratis`

**Post 2 (06/05) — Carrusel:**
> Slide 1: Agregá tus egresos con acreedor, monto y fecha de vencimiento.
> Slide 2: Navegá entre meses para ver lo que viene.
> Slide 3: Mirá tu resumen: total a pagar, por moneda (ARS y USD).
>
> Sin complicaciones. Sin formularios interminables.
>
> `#Nivva #finanzaspersonales #egresos #cuotas #presupuesto #Argentina`

**Post 3 (08/05):**
> 'Sí, sí, me acuerdo que vencía el 10... o era el 15?' 🫠
>
> Si te pasó, no estás solo/a. Muchas personas usan la memoria como único método de control financiero.
>
> Nivva te avisa antes de que venza — para que no te agarre por sorpresa.
>
> `#finanzaspersonales #vencimientos #recordatorios #Nivva #Argentina #cuotas`

---

### Semana 2 — 11 al 15 de mayo de 2026
> **Foco:** Alta rápida de deudas, educación sobre tipos de egresos y recordatorios

| # | Fecha | Día | Pilar | Formato | Hook | CTA | Persona objetivo |
|---|---|---|---|---|---|---|---|
| 4 | 11/05 | Lunes | Funcionalidad destacada | Reel (15-30 seg) | Registrá una deuda en menos de 30 segundos | Probalo vos mismo → link en bio. ¿Algo no funciona como esperabas? Escribinos 💬 | Ana / Facundo |
| 5 | 13/05 | Miércoles | Educación financiera simple | Carrusel (2 slides) | ¿Egreso fijo o variable? La diferencia cambia todo | Guardá para tenerlo de referencia 💡 | Ana / Claudia |
| 6 | 15/05 | Viernes | Funcionalidad destacada | Post estático | Que el vencimiento no te agarre de sorpresa 🔔 | ¿Tenés activadas las notificaciones? → link en bio | Ana / Claudia |

**Captions desarrollados:**

**Post 4 (11/05) — Reel:**
> Con Nivva podés cargar:
> ✔️ Acreedor
> ✔️ Monto en ARS o USD
> ✔️ Fecha de vencimiento
> ✔️ Cuántas cuotas quedan
>
> Sin complicaciones. Sin pasos de más.
>
> Si algo no te sale, contanos — lo resolvemos 💬
>
> `#Nivva #finanzas #deudas #ARS #USD #controldedeudas #Argentina`

**Post 5 (13/05) — Carrusel:**
> 📌 Egreso fijo: monto constante todos los meses. Alquiler, cuota del seguro, servicio de streaming.
>
> 📌 Egreso variable: monto que cambia. La boleta de luz, el súper, salir a comer.
>
> Registrar los fijos primero te da una base sólida para planificar. Nivva te ayuda a hacerlo.
>
> `#finanzaspersonales #educacionfinanciera #egresos #Nivva #Argentina #presupuesto`

**Post 6 (15/05):**
> Nivva te manda un recordatorio antes de que venza tu próximo pago.
>
> Sin spam. Sin interrupciones. Solo cuando importa.
>
> Activá las notificaciones y nunca más llegues tarde a un pago.
>
> `#Nivva #recordatorios #vencimientos #finanzas #notificaciones #Argentina`

---

### Semana 3 — 18 al 22 de mayo de 2026
> **Foco:** Ingresos, balance mensual y privacidad de datos

| # | Fecha | Día | Pilar | Formato | Hook | CTA | Persona objetivo |
|---|---|---|---|---|---|---|---|
| 7 | 18/05 | Lunes | Funcionalidad destacada | Carrusel (2 slides) | No solo lo que debés — también lo que ganás | ¿Registrás tus ingresos? Contanos cómo lo hacés 👇 | Ana / Facundo |
| 8 | 20/05 | Miércoles | Educación financiera simple | Post estático | Ingresos − Egresos = lo que realmente te queda | Empezá a registrar hoy → link en bio | Facundo / Claudia |
| 9 | 22/05 | Viernes | Privacidad y confianza | Post estático | Tus datos financieros no son de nadie más 🔐 | ¿Tenés alguna duda o algo que no te queda claro? Escribinos por DM 💬 | Ana / Claudia / Facundo |

**Captions desarrollados:**

**Post 7 (18/05) — Carrusel:**
> Además de tus egresos, con Nivva podés registrar tus ingresos del mes.
>
> Sueldo, freelance, changas, alquileres — lo que sea.
>
> Así podés ver tu balance real: cuánto entra, cuánto sale, qué te queda.
>
> `#Nivva #ingresos #balance #finanzaspersonales #Argentina #freelance`

**Post 8 (20/05):**
> Suena obvio, pero el 49 % de las personas no lleva ningún registro.
>
> Saber cuánto te sobra (o cuánto te falta) al final del mes no es magia — es una resta.
>
> Nivva la hace por vos.
>
> `#finanzaspersonales #balance #educacionfinanciera #Nivva #Argentina #control`

**Post 9 (22/05):**
> Con Nivva, tus datos se guardan en tu dispositivo.
>
> Nada en la nube. Ningún servidor externo. Tu información no se sube ni se comparte con servidores externos.
>
> Vos controlás todo.
>
> ¿Algo no te queda claro o encontraste algo raro? Contanos por DM — estamos mejorando la app y tu feedback cuenta 🛠️
>
> `#Nivva #privacidad #seguridaddigital #finanzaspersonales #datos #Argentina`

---

### Semana 4 — 25 al 29 de mayo de 2026
> **Foco:** Inversiones, cierre de mes relatable y recap final con CTA

| # | Fecha | Día | Pilar | Formato | Hook | CTA | Persona objetivo |
|---|---|---|---|---|---|---|---|
| 10 | 25/05 | Lunes | Funcionalidad destacada | Carrusel (3 slides) | ¿Invertís? Llevá el registro de todo en un solo lugar | ¿Ya usás la sección de inversiones? → link en bio | Ana |
| 11 | 27/05 | Miércoles | Identificación con el usuario | Reel (15 seg) | '¿A dónde fueron tus gastos?' — Historia universal | ¿Lo viviste? Dale ❤️ si te pasó | Facundo / Claudia |
| 12 | 29/05 | Viernes | Recap y CTA | Carrusel (4 slides) | Un mes de Nivva: todo lo que podés hacer, gratis | Probala gratis → link en bio | Ana / Facundo / Claudia |

**Captions desarrollados:**

**Post 10 (25/05) — Carrusel:**
> Además de egresos e ingresos, Nivva tiene una sección de Inversiones.
>
> Registrá plazo fijo, cripto, dólares, acciones — lo que sea.
>
> Mirá el total invertido por moneda y seguí cómo evoluciona con cada nuevo valor que cargás.
>
> `#Nivva #inversiones #finanzaspersonales #plazoFijo #Argentina #ahorro #cripto`

**Post 11 (27/05) — Reel:**
> Fin de mes. Revisás la cuenta y no entendés nada.
>
> La tarjeta, el alquiler, la luz, las cuotas de no-sé-qué...
>
> No es que gastaste de más — es que no lo tenías registrado.
>
> Nivva es el primer paso para empezar a saber.
>
> `#Nivva #findemes #finanzaspersonales #Argentina #gastosdelmes`

**Post 12 (29/05) — Carrusel resumen:**
> Registrá egresos y cuotas 📋
> Seguí tus ingresos 💵
> Controlá tus inversiones 📈
> Recibí recordatorios de vencimientos 🔔
> Exportá e importá tus datos 💾
> Todo sin que nada salga de tu dispositivo 🔐
>
> Gratis. Sin cuenta. Sin suscripción.
>
> Empezá hoy → link en bio
>
> ¿Encontraste algo que no funciona como esperabas? Contanos por DM — tu reporte nos ayuda a mejorar 🛠️
>
> `#Nivva #finanzaspersonales #app #gratis #Argentina #controlfinanciero #balance`

---

## 📅 Vista agrupada por Mes y Año

| Mes/Año | Semana | Fecha | Pilar | Formato | Persona objetivo |
|---|---|---|---|---|---|
| Mayo 2026 | Semana 1 | 04/05 | Presentación de marca | Reel (8–12 seg) | Ana / Claudia |
| Mayo 2026 | Semana 1 | 06/05 | Funcionalidad destacada | Carrusel | Facundo |
| Mayo 2026 | Semana 1 | 08/05 | Identificación con el usuario | Post estático | Facundo / Claudia |
| Mayo 2026 | Semana 2 | 11/05 | Funcionalidad destacada | Reel | Ana / Facundo |
| Mayo 2026 | Semana 2 | 13/05 | Educación financiera simple | Carrusel | Ana / Claudia |
| Mayo 2026 | Semana 2 | 15/05 | Funcionalidad destacada | Post estático | Ana / Claudia |
| Mayo 2026 | Semana 3 | 18/05 | Funcionalidad destacada | Carrusel | Ana / Facundo |
| Mayo 2026 | Semana 3 | 20/05 | Educación financiera simple | Post estático | Facundo / Claudia |
| Mayo 2026 | Semana 3 | 22/05 | Privacidad y confianza | Post estático | Ana / Claudia / Facundo |
| Mayo 2026 | Semana 4 | 25/05 | Funcionalidad destacada | Carrusel | Ana |
| Mayo 2026 | Semana 4 | 27/05 | Identificación con el usuario | Reel | Facundo / Claudia |
| Mayo 2026 | Semana 4 | 29/05 | Recap y CTA | Carrusel | Ana / Facundo / Claudia |

### Distribución por pilar — Mayo 2026

| Pilar | Cantidad de posts | % del mes |
|---|---|---|
| Funcionalidad destacada | 5 | 42 % |
| Educación financiera simple | 2 | 17 % |
| Identificación con el usuario | 2 | 17 % |
| Presentación de marca | 1 | 8 % |
| Privacidad y confianza | 1 | 8 % |
| Recap y CTA | 1 | 8 % |

### Distribución por formato — Mayo 2026

| Formato | Cantidad |
|---|---|
| Carrusel | 5 |
| Post estático | 4 |
| Reel | 3 |

### Distribución por persona objetivo — Mayo 2026

| Persona | Posts donde aparece |
|---|---|
| Ana (Administradora Consciente) | 8 |
| Facundo (Freelance Desbordado) | 7 |
| Claudia (Equilibrista Ocupada) | 8 |

---

## 🎨 Notas de diseño y marca

| Elemento | Especificación |
|---|---|
| **Color primario** | Teal Nivva `#3D8F8F` — botones, íconos, acentos |
| **Fondo principal** | Crema Nivva `#F2EDE8` — tarjetas, fondos limpios |
| **Texto** | Negro suave `#1A1A1A` — headings y cuerpo |
| **Texto secundario** | Gris medio `#6B7280` — labels y metadatos |
| **Tipografía** | Sans-serif bold para hooks; sans-serif regular para cuerpo |
| **Logo** | Símbolo N+V (escudo teal) + wordmark NIVVA en negro |
| **Tagline** | "Tus números, fácil y simple" |
| **Registro** | Siempre "vos" (rioplatense). Nunca "tú". |

---

## 🔁 Reglas de copy aplicadas

Basadas en el [Manual de Voz y Tono de Nivva (04-Voz-y-tono)](../wiki/04-Voz-y-tono.md):

- ✅ Registro "vos" en todos los captions
- ✅ Frases cortas (máximo 2 cláusulas por oración)
- ✅ Sin tecnicismos ni anglicismos expuestos
- ✅ Estructura: qué le da la app + próximo paso (CTA)
- ✅ No se promete "libertad financiera" ni "control total"
- ✅ Hashtags en español y neutros, con `#Argentina` para geolocalización
- ✅ Emojis principalmente en posts relacionables/Reels; en posts institucionales, solo uso puntual y funcional (por ejemplo, para enfatizar o señalizar), evitando un tono recargado
- ✅ CTA de feedback en posts de funcionalidad y cierre: invitar a reportar errores o dudas por DM ("Si algo no te sale, contanos — lo resolvemos")

---

## 📎 Archivos relacionados

- [`../wiki/00-Marca.md`](../wiki/00-Marca.md) — Identidad visual y propósito de Nivva
- [`../wiki/04-Voz-y-tono.md`](../wiki/04-Voz-y-tono.md) — Reglas de registro y copy
- [`../wiki/09-Personas.md`](../wiki/09-Personas.md) — Perfiles de usuario (Ana, Facundo, Claudia)
