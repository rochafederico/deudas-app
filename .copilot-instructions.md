...
- La función `addOrMergeDeuda` está implementada en `deudaRepository` y busca deudas existentes por **acreedor + tipoDeuda**. Si encuentra una coincidencia, obtiene los montos actuales y fusiona solo los montos que no estén duplicados (comparación por `monto`, `moneda` y `periodo`/`vencimiento`). Retorna el id de la deuda ya existente o el nuevo id creado.
